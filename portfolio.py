...
class Portfolio(IEventListener):
    ...
    def update(self, *args):
        ...
        if type(args[1]) == pd.DataFrame:
            ...
            if (
                self.optionType.__contains__(token)
                and price >= self.strike_price[token]
            ):
                self.sell(token)
            ...
            optionGeek = OptionGeeks(
                token, OptionType.call, underlying_price=price
            )
            optionGeek2 = OptionGeeks(
                token, OptionType.put, underlying_price=price
            )
